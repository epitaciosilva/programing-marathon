...
int levels, nprob = 0, start = 0, inicio = 0, marcador = 100000, contLevels = 0, removidos = 0, nIndexVal = 0;
int probs[MAX], aux[MAX] = {0}, indexVal[MAX] = {-1};

bool temSolucao() {
    if(nIndexVal > 0) {
        for(int i = 0; i < nIndexVal; i++) {
            // se o número ainda não está dentro de aux, adiciona
    		if(aux[probs[indexVal[i]]-1] != probs[indexVal[i]]) {
	    		aux[probs[indexVal[i]]-1] = probs[indexVal[i]];
		    	contLevels++;
    		} else if(i <= marcador){
